...
// hong
int TableScanIterator::Read_Custom() {

  int tmp;

  if (table()->is_union_or_table()) {
    
    clock_gettime(CLOCK_MONOTONIC, &the_time_5[0]);

    while (true) {
      //printf("loop point1 \n");
      clock_gettime(CLOCK_MONOTONIC, &the_time_6[0]);

      tmp = table()->file->ha_rnd_next(m_record);

      clock_gettime(CLOCK_MONOTONIC, &the_time_6[1]);

      Time_Check(the_time_6, 
    ...
    clock_gettime(CLOCK_MONOTONIC, &the_time_5[1]);

    Time_Check(the_time_5, 
...
int TableScanIterator::Read_Original() {
    int tmp;
  if (table()->is_union_or_table()) {
