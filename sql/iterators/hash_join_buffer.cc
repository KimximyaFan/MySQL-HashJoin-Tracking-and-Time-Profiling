...
//unsigned long long int hash_join_row_buffer_store_row_count;  // hong
...
// hong
LinkedImmutableString HashJoinRowBuffer::StoreLinkedImmutableStringFromTableBuffers(
  LinkedImmutableString next_ptr, bool *full) {
  
  clock_gettime(CLOCK_MONOTONIC, &the_time_2[0]);
  LinkedImmutableString temp = StoreLinkedImmutableStringFromTableBuffers_Original(next_ptr, full);
  clock_gettime(CLOCK_MONOTONIC, &the_time_2[1]);

  Time_Check(the_time_2, &hash_join_buffer_store_row_time, &buffer_store_immutable_string_count);
...
HashJoinRowBuffer::StoreLinkedImmutableStringFromTableBuffers_Original(
    LinkedImmutableString next_ptr, bool *full) {
  //printf("hash_join_buffer.cc\n class HashJoinRowBuffer - StoreLinkedImmutableStringFromTableBuffers() -> LinkedImmutableString \n"); // hong
...
HashJoinRowBuffer::HashJoinRowBuffer(
  ...
  //printf("hash_join_buffer.cc\n class HashJoinRowBuffer - HashJoinRowBuffer() .. Constructor \n"); // hong
...
StoreRowResult HashJoinRowBuffer::StoreRow(THD *thd,
  ...
  clock_gettime(CLOCK_MONOTONIC, &the_time_2[0]);
  StoreRowResult temp_result = StoreRow_Original(thd, reject_duplicate_keys);
  clock_gettime(CLOCK_MONOTONIC, &the_time_2[1]);

  Time_Check(the_time_2, &hash_join_buffer_store_row_time, &buffer_store_row_count);

  return temp_result;
}

// hong
StoreRowResult HashJoinRowBuffer::StoreRow_Original(THD *thd,
                                           bool reject_duplicate_keys) {
  
  //Count_Up(&hash_join_row_buffer_store_row_count); // hong
  //printf("hash_join_buffer.cc\n class HashJoinRowBuffer - StoreRow() -> StoreRowResult \n"); // hong
...
std::pair<const char *, uint64_t> VarintParseSlow64(const char *p,
  ...
  //printf("hash_join_buffer.cc\nVarintParseSlow64() -> pair<const char *, uint64_t> \n"); // hong
