...
bool Sql_cmd_dml::execute(THD *thd) {
  printf("sql_select.cc\n class Sql_cmd_dml - execute() \n"); // hong
  DBUG_TRACE;
...
bool Sql_cmd_dml::execute_inner(THD *thd) {
  printf("sql_select.cc\n class Sql_cmd_dml - execute_inner() \n"); // hong
  ...
  if (lex->is_explain()) {
    printf("class Sql_cmd_dml - execute_inner() ... 1 \n"); // hong
    if (explain_query(thd, thd, unit)) return true; /* purecov: inspected */
  } else {
    printf("class Sql_cmd_dml - execute_inner() ... 2 \n"); // hong
