                     ...
                     ROW_SEL_EXACT_PREFIX */
{
  ...
  if (!intrinsic) {
    clock_gettime(CLOCK_MONOTONIC, &the_time_9[0]);
    ret = row_search_mvcc(buf, PAGE_CUR_UNSUPP, m_prebuilt, match_mode,
                          direction);
    clock_gettime(CLOCK_MONOTONIC, &the_time_9[1]);
    Time_Check(the_time_9, 
                                      ...
                                      buffer, in MySQL format */
{
  ...
  if (m_start_of_scan) {
    clock_gettime(CLOCK_MONOTONIC, &the_time_8[0]);
    error = index_first(buf);
    clock_gettime(CLOCK_MONOTONIC, &the_time_8[1]);
    Time_Check(the_time_8, 
