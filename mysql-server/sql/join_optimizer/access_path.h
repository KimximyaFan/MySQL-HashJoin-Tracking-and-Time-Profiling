...
struct AccessPath {
  ...
  double rescan_cost() const { 
    //printf("access_path.h\n struct AccessPath - rescan_cost() -> double \n"); // hong
    return cost - init_once_cost; }
  ...
  OverflowBitset &applied_sargable_join_predicates() {
    //printf("access_path.h\n struct AccessPath - &applied_sargable_join_predicates() -> OverflowBitset \n"); // hong
    return filter_predicates;
  }
  const OverflowBitset &applied_sargable_join_predicates() const {
    //printf("access_path.h\n struct AccessPath - &applied_sargable_join_predicates() -> OverflowBitset .. Overloading\n"); // hong
    return filter_predicates;
  }
  ...
  OverflowBitset &subsumed_sargable_join_predicates() {
    //printf("access_path.h\n struct AccessPath - &subsumed_sargable_join_predicates() -> OverflowBitset \n"); // hong
    return delayed_predicates;
  }
  const OverflowBitset &subsumed_sargable_join_predicates() const {
    //printf("access_path.h\n struct AccessPath - &subsumed_sargable_join_predicates() -> OverflowBitset .. Overloading\n"); // hong
    return delayed_predicates;
  }
  ...
  auto &table_scan() {
    //printf("access_path.h\n struct AccessPath - &table_scan() -> auto\n"); // hong
    assert(type == TABLE_SCAN);
    return u.table_scan;
  }
  const auto &table_scan() const {
    //printf("access_path.h\n struct AccessPath - &table_scan() -> auto .. Overloading \n"); // hong
    assert(type == TABLE_SCAN);
    return u.table_scan;
  }
  auto &index_scan() {
    //printf("access_path.h\n struct AccessPath - &index_scan() -> auto \n"); // hong
    assert(type == INDEX_SCAN);
    return u.index_scan;
  }
  const auto &index_scan() const {
    //printf("access_path.h\n struct AccessPath - &index_scan() -> auto .. Overloading \n"); // hong
    assert(type == INDEX_SCAN);
    return u.index_scan;
  }
  auto &ref() {
    //printf("access_path.h\n struct AccessPath - &ref() -> auto \n"); // hong
    assert(type == REF);
    return u.ref;
  }
  const auto &ref() const {
    //printf("access_path.h\n struct AccessPath - &ref() -> auto .. Overloading \n"); // hong
    assert(type == REF);
    return u.ref;
  }
  auto &ref_or_null() {
    //printf("access_path.h\n struct AccessPath - &ref_or_null() -> auto \n"); // hong
    assert(type == REF_OR_NULL);
    return u.ref_or_null;
  }
  const auto &ref_or_null() const {
    //printf("access_path.h\n struct AccessPath - &ref_or_null() -> auto .. Overloading \n"); // hong
    assert(type == REF_OR_NULL);
    return u.ref_or_null;
  }
  auto &eq_ref() {
    //printf("access_path.h\n struct AccessPath - &eq_ref() -> auto \n"); // hong
    assert(type == EQ_REF);
    return u.eq_ref;
  }
  const auto &eq_ref() const {
    //printf("access_path.h\n struct AccessPath - &eq_ref() -> auto .. Overloading \n"); // hong
    assert(type == EQ_REF);
    return u.eq_ref;
  }
  auto &pushed_join_ref() {
    //printf("access_path.h\n struct AccessPath - &pushed_join_ref() -> auto \n"); // hong
    assert(type == PUSHED_JOIN_REF);
    return u.pushed_join_ref;
  }
  const auto &pushed_join_ref() const {
    //printf("access_path.h\n struct AccessPath - &pushed_join_ref() -> auto .. Overloading \n"); // hong
    assert(type == PUSHED_JOIN_REF);
    return u.pushed_join_ref;
  }
  auto &full_text_search() {
    //printf("access_path.h\n struct AccessPath - &full_text_search() -> auto \n"); // hong
    assert(type == FULL_TEXT_SEARCH);
    return u.full_text_search;
  }
  const auto &full_text_search() const {
    //printf("access_path.h\n struct AccessPath - &full_text_search() -> auto .. Overloading \n"); // hong
    assert(type == FULL_TEXT_SEARCH);
    return u.full_text_search;
  }
  auto &const_table() {
    //printf("access_path.h\n struct AccessPath - &const_table() -> auto \n"); // hong
    assert(type == CONST_TABLE);
    return u.const_table;
  }
  const auto &const_table() const {
    //printf("access_path.h\n struct AccessPath - &const_table() -> auto .. Overloading \n"); // hong
    assert(type == CONST_TABLE);
    return u.const_table;
  }
  auto &mrr() {
    //printf("access_path.h\n struct AccessPath - &mrr() -> auto \n"); // hong
    assert(type == MRR);
    return u.mrr;
  }
  const auto &mrr() const {
    //printf("access_path.h\n struct AccessPath - &mrr() -> auto .. Overloading \n"); // hong
    assert(type == MRR);
    return u.mrr;
  }
  auto &follow_tail() {
    //printf("access_path.h\n struct AccessPath - &follow_tail() -> auto \n"); // hong
    assert(type == FOLLOW_TAIL);
    return u.follow_tail;
  }
  const auto &follow_tail() const {
    //printf("access_path.h\n struct AccessPath - &follow_tail() -> auto .. Overloading \n"); // hong
    assert(type == FOLLOW_TAIL);
    return u.follow_tail;
  }
  auto &index_range_scan() {
    //printf("access_path.h\n struct AccessPath - &index_range_scan() -> auto \n"); // hong
    assert(type == INDEX_RANGE_SCAN);
    return u.index_range_scan;
  }
  const auto &index_range_scan() const {
    //printf("access_path.h\n struct AccessPath - &index_range_scan() -> auto .. Overloading \n"); // hong
    assert(type == INDEX_RANGE_SCAN);
    return u.index_range_scan;
  }
  auto &index_merge() {
    //printf("access_path.h\n struct AccessPath - &index_merge() -> auto \n"); // hong
    assert(type == INDEX_MERGE);
    return u.index_merge;
  }
  const auto &index_merge() const {
    //printf("access_path.h\n struct AccessPath - &index_merge() -> auto .. Overloading \n"); // hong
    assert(type == INDEX_MERGE);
    return u.index_merge;
  }
  auto &rowid_intersection() {
    //printf("access_path.h\n struct AccessPath - &rowid_intersection() -> auto \n"); // hong
    assert(type == ROWID_INTERSECTION);
    return u.rowid_intersection;
  }
  const auto &rowid_intersection() const {
    //printf("access_path.h\n struct AccessPath - &rowid_intersection() -> auto .. Overloading \n"); // hong
    assert(type == ROWID_INTERSECTION);
    return u.rowid_intersection;
  }
  auto &rowid_union() {
    //printf("access_path.h\n struct AccessPath - &rowid_union() -> auto \n"); // hong
    assert(type == ROWID_UNION);
    return u.rowid_union;
  }
  const auto &rowid_union() const {
    //printf("access_path.h\n struct AccessPath - &rowid_union() -> auto .. Overloading \n"); // hong
    assert(type == ROWID_UNION);
    return u.rowid_union;
  }
  auto &index_skip_scan() {
    //printf("access_path.h\n struct AccessPath - &index_skip_scan() -> auto  \n"); // hong
    assert(type == INDEX_SKIP_SCAN);
    return u.index_skip_scan;
  }
  const auto &index_skip_scan() const {
    //printf("access_path.h\n struct AccessPath - &index_skip_scan() -> auto .. Overloading \n"); // hong
    assert(type == INDEX_SKIP_SCAN);
    return u.index_skip_scan;
  }
  auto &group_index_skip_scan() {
    //printf("access_path.h\n struct AccessPath - &group_index_skip_scan() -> auto  \n"); // hong
    assert(type == GROUP_INDEX_SKIP_SCAN);
    return u.group_index_skip_scan;
  }
  const auto &group_index_skip_scan() const {
    //printf("access_path.h\n struct AccessPath - &group_index_skip_scan() -> auto .. Overloading \n"); // hong
    assert(type == GROUP_INDEX_SKIP_SCAN);
    return u.group_index_skip_scan;
  }
  auto &dynamic_index_range_scan() {
    //printf("access_path.h\n struct AccessPath - &dynamic_index_range_scan() -> auto \n"); // hong
    assert(type == DYNAMIC_INDEX_RANGE_SCAN);
    return u.dynamic_index_range_scan;
  }
  const auto &dynamic_index_range_scan() const {
    //printf("access_path.h\n struct AccessPath - &dynamic_index_range_scan() -> auto .. Overloading \n"); // hong
    assert(type == DYNAMIC_INDEX_RANGE_SCAN);
    return u.dynamic_index_range_scan;
  }
  auto &materialized_table_function() {
    //printf("access_path.h\n struct AccessPath - &materialized_table_function() -> auto .. Overloading \n"); // hong
  ...
  auto &unqualified_count() {
    //printf("access_path.h\n struct AccessPath - &unqualified_count() -> auto  \n"); // hong
    assert(type == UNQUALIFIED_COUNT);
    return u.unqualified_count;
  }
  const auto &unqualified_count() const {
    //printf("access_path.h\n struct AccessPath - &unqualified_count() -> auto .. Overloading \n"); // hong
    assert(type == UNQUALIFIED_COUNT);
    return u.unqualified_count;
  }
  auto &table_value_constructor() {
    //printf("access_path.h\n struct AccessPath - &table_value_constructor() -> auto \n"); // hong
    assert(type == TABLE_VALUE_CONSTRUCTOR);
    return u.table_value_constructor;
  }
  const auto &table_value_constructor() const {
    //printf("access_path.h\n struct AccessPath - &table_value_constructor() -> auto .. Overloading \n"); // hong
    assert(type == TABLE_VALUE_CONSTRUCTOR);
    return u.table_value_constructor;
  }
  auto &fake_single_row() {
    //printf("access_path.h\n struct AccessPath - &fake_single_row() -> auto \n"); // hong
    assert(type == FAKE_SINGLE_ROW);
    return u.fake_single_row;
  }
  const auto &fake_single_row() const {
    //printf("access_path.h\n struct AccessPath - &fake_single_row() -> auto .. Overloading \n"); // hong
    assert(type == FAKE_SINGLE_ROW);
    return u.fake_single_row;
  }
  auto &zero_rows() {
    //printf("access_path.h\n struct AccessPath - &zero_rows() -> auto \n"); // hong
    assert(type == ZERO_ROWS);
    return u.zero_rows;
  }
  const auto &zero_rows() const {
    //printf("access_path.h\n struct AccessPath - &zero_rows() -> auto .. Overloading \n"); // hong
    assert(type == ZERO_ROWS);
    return u.zero_rows;
  }
  auto &zero_rows_aggregated() {
    //printf("access_path.h\n struct AccessPath - &zero_rows_aggregated() -> auto \n"); // hong
    assert(type == ZERO_ROWS_AGGREGATED);
    return u.zero_rows_aggregated;
  }
  const auto &zero_rows_aggregated() const {
    //printf("access_path.h\n struct AccessPath - &zero_rows_aggregated() -> auto .. Overloading \n"); // hong
    assert(type == ZERO_ROWS_AGGREGATED);
    return u.zero_rows_aggregated;
  }
  auto &hash_join() {
    //printf("access_path.h\n struct AccessPath - &hash_join() -> auto \n"); // hong
    assert(type == HASH_JOIN);
    return u.hash_join;
  }
  const auto &hash_join() const {
    //printf("access_path.h\n struct AccessPath - &hash_join() -> auto .. Overloading \n"); // hong
    assert(type == HASH_JOIN);
    return u.hash_join;
  }
  auto &bka_join() {
    //printf("access_path.h\n struct AccessPath - &bka_join() -> auto  \n"); // hong
    assert(type == BKA_JOIN);
    return u.bka_join;
  }
  const auto &bka_join() const {
    //printf("access_path.h\n struct AccessPath - &bka_join() -> auto .. Overloading \n"); // hong
    assert(type == BKA_JOIN);
    return u.bka_join;
  }
  auto &nested_loop_join() {
    //printf("access_path.h\n struct AccessPath - &nested_loop_join() -> auto \n"); // hong
    assert(type == NESTED_LOOP_JOIN);
    return u.nested_loop_join;
  }
  const auto &nested_loop_join() const {
    //printf("access_path.h\n struct AccessPath - &nested_loop_join() -> auto .. Overloading \n"); // hong
    assert(type == NESTED_LOOP_JOIN);
    return u.nested_loop_join;
  }
  auto &nested_loop_semijoin_with_duplicate_removal() {
    //printf("access_path.h\n struct AccessPath - &nested_loop_semijoin_with_duplicate_removal() -> auto \n"); // hong
  ...
  const auto &nested_loop_semijoin_with_duplicate_removal() const {
    //printf("access_path.h\n struct AccessPath - &nested_loop_semijoin_with_duplicate_removal() -> auto .. Overloading \n"); // hong
    assert(type == NESTED_LOOP_SEMIJOIN_WITH_DUPLICATE_REMOVAL);
    return u.nested_loop_semijoin_with_duplicate_removal;
  }
  auto &filter() {
    //printf("access_path.h\n struct AccessPath - &filter() -> auto \n"); // hong
    assert(type == FILTER);
    return u.filter;
  }
  const auto &filter() const {
    //printf("access_path.h\n struct AccessPath - &filter() -> auto .. Overloading \n"); // hong
    assert(type == FILTER);
    return u.filter;
  }
  auto &sort() {
    //printf("access_path.h\n struct AccessPath - &sort() -> auto \n"); // hong
    assert(type == SORT);
    return u.sort;
  }
  const auto &sort() const {
    //printf("access_path.h\n struct AccessPath - &sort() -> auto .. Overloading \n"); // hong
    assert(type == SORT);
    return u.sort;
  }
  auto &aggregate() {
    //printf("access_path.h\n struct AccessPath - &aggregate() -> auto \n"); // hong
    assert(type == AGGREGATE);
    return u.aggregate;
  }
  const auto &aggregate() const {
    //printf("access_path.h\n struct AccessPath - &aggregate() -> auto .. Overloading \n"); // hong
    assert(type == AGGREGATE);
    return u.aggregate;
  }
  auto &temptable_aggregate() {
    //printf("access_path.h\n struct AccessPath - &temptable_aggregate() -> auto \n"); // hong
    assert(type == TEMPTABLE_AGGREGATE);
    return u.temptable_aggregate;
  }
  const auto &temptable_aggregate() const {
    //printf("access_path.h\n struct AccessPath - &temptable_aggregate() -> auto .. Overloading \n"); // hong
    assert(type == TEMPTABLE_AGGREGATE);
    return u.temptable_aggregate;
  }
  auto &limit_offset() {
    //printf("access_path.h\n struct AccessPath - &limit_offset() -> auto \n"); // hong
    assert(type == LIMIT_OFFSET);
    return u.limit_offset;
  }
  const auto &limit_offset() const {
    //printf("access_path.h\n struct AccessPath - &limit_offset() -> auto .. Overloading \n"); // hong
    assert(type == LIMIT_OFFSET);
    return u.limit_offset;
  }
  auto &stream() {
    //printf("access_path.h\n struct AccessPath - &stream() -> auto \n"); // hong
    assert(type == STREAM);
    return u.stream;
  }
  const auto &stream() const {
    //printf("access_path.h\n struct AccessPath - &stream() -> auto .. Overloading \n"); // hong
    assert(type == STREAM);
    return u.stream;
  }
  auto &materialize() {
    //printf("access_path.h\n struct AccessPath - &materialize() -> auto \n"); // hong
    assert(type == MATERIALIZE);
    return u.materialize;
  }
  const auto &materialize() const {
    //printf("access_path.h\n struct AccessPath - &materialize() -> auto .. Overloading \n"); // hong
    assert(type == MATERIALIZE);
    return u.materialize;
  }
  auto &materialize_information_schema_table() {
    //printf("access_path.h\n struct AccessPath - &materialize_information_schema_table() -> auto \n"); // hong
  ...
  const auto &materialize_information_schema_table() const {
    //printf("access_path.h\n struct AccessPath - &materialize_information_schema_table() -> auto .. Overloading \n"); // hong
    assert(type == MATERIALIZE_INFORMATION_SCHEMA_TABLE);
    return u.materialize_information_schema_table;
  }
  auto &append() {
    //printf("access_path.h\n struct AccessPath - &append() -> auto \n"); // hong
    assert(type == APPEND);
    return u.append;
  }
  const auto &append() const {
    //printf("access_path.h\n struct AccessPath - &append() -> auto .. Overloading \n"); // hong
    assert(type == APPEND);
    return u.append;
  }
  auto &window() {
    //printf("access_path.h\n struct AccessPath - &window() -> auto \n"); // hong
    assert(type == WINDOW);
    return u.window;
  }
  const auto &window() const {
    //printf("access_path.h\n struct AccessPath - &window() -> auto .. Overloading \n"); // hong
    assert(type == WINDOW);
    return u.window;
  }
  auto &weedout() {
    //printf("access_path.h\n struct AccessPath - &weedout() -> auto \n"); // hong
    assert(type == WEEDOUT);
    return u.weedout;
  }
  const auto &weedout() const {
    //printf("access_path.h\n struct AccessPath - &weedout() -> auto .. Overloading \n"); // hong
    assert(type == WEEDOUT);
    return u.weedout;
  }
  auto &remove_duplicates() {
    //printf("access_path.h\n struct AccessPath - &remove_duplicates() -> auto \n"); // hong
    assert(type == REMOVE_DUPLICATES);
    return u.remove_duplicates;
  }
  const auto &remove_duplicates() const {
    //printf("access_path.h\n struct AccessPath - &remove_duplicates() -> auto .. Overloading \n"); // hong
    assert(type == REMOVE_DUPLICATES);
    return u.remove_duplicates;
  }
  auto &remove_duplicates_on_index() {
    //printf("access_path.h\n struct AccessPath - &remove_duplicates_on_index() -> auto \n"); // hong
    assert(type == REMOVE_DUPLICATES_ON_INDEX);
    return u.remove_duplicates_on_index;
  }
  const auto &remove_duplicates_on_index() const {
    //printf("access_path.h\n struct AccessPath - &remove_duplicates_on_index() -> auto .. Overloading \n"); // hong
    assert(type == REMOVE_DUPLICATES_ON_INDEX);
    return u.remove_duplicates_on_index;
  }
  auto &alternative() {
    //printf("access_path.h\n struct AccessPath - &alternative() -> auto \n"); // hong
    assert(type == ALTERNATIVE);
    return u.alternative;
  }
  const auto &alternative() const {
    //printf("access_path.h\n struct AccessPath - &alternative() -> auto .. Overloading \n"); // hong
    assert(type == ALTERNATIVE);
    return u.alternative;
  }
  auto &cache_invalidator() {
    //printf("access_path.h\n struct AccessPath - &cache_invalidator() -> auto \n"); // hong
    assert(type == CACHE_INVALIDATOR);
    return u.cache_invalidator;
  }
  const auto &cache_invalidator() const {
    //printf("access_path.h\n struct AccessPath - &cache_invalidator() -> auto .. Overloading \n"); // hong
    assert(type == CACHE_INVALIDATOR);
    return u.cache_invalidator;
  }
  auto &delete_rows() {
    //printf("access_path.h\n struct AccessPath - &delete_rows() -> auto \n"); // hong
    assert(type == DELETE_ROWS);
    return u.delete_rows;
  }
  const auto &delete_rows() const {
    //printf("access_path.h\n struct AccessPath - &delete_rows() -> auto .. Overloading \n"); // hong
    assert(type == DELETE_ROWS);
    return u.delete_rows;
  }
  auto &update_rows() {
    //printf("access_path.h\n struct AccessPath - &update_rows() -> auto \n"); // hong
    assert(type == UPDATE_ROWS);
    return u.update_rows;
  }
  const auto &update_rows() const {
    //printf("access_path.h\n struct AccessPath - &update_rows() -> auto .. Overloading \n"); // hong
    assert(type == UPDATE_ROWS);
    return u.update_rows;
  }

  double num_output_rows() const { 
    //printf("access_path.h\n struct AccessPath - num_output_rows() -> double \n"); // hong
    return m_num_output_rows; }

  void set_num_output_rows(double val) { 
    //printf("access_path.h\n struct AccessPath - set_num_output_rows() \n"); // hong
...
inline AccessPath *NewTableScanAccessPath(THD *thd, TABLE *table,
  ...
  //printf("access_path.h\n *NewTableScanAccessPath() -> AccessPath \n"); // hong
...
inline AccessPath *NewIndexScanAccessPath(THD *thd, TABLE *table, int idx,
                                          ...
                                          bool count_examined_rows) {
  //printf("access_path.h\n *NewIndexScanAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewRefOrNullAccessPath(THD *thd, TABLE *table,
                                          ...
                                          bool count_examined_rows) {
  //printf("access_path.h\n *NewRefOrNullAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewPushedJoinRefAccessPath(THD *thd, TABLE *table,
                                              ...
                                              bool is_unique,
                                              bool count_examined_rows) {
  //printf("access_path.h\n *NewPushedJoinRefAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewFullTextSearchAccessPath(THD *thd, TABLE *table,
                                               ...
                                               bool count_examined_rows) {
  //printf("access_path.h\n *NewFullTextSearchAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewConstTableAccessPath(THD *thd, TABLE *table,
                                           Index_lookup *ref,
                                           bool count_examined_rows) {
  //printf("access_path.h\n *NewConstTableAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewFollowTailAccessPath(THD *thd, TABLE *table,
                                           bool count_examined_rows) {
  //printf("access_path.h\n *NewFollowTailAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewDynamicIndexRangeScanAccessPath(
  ...
  //printf("access_path.h\n *NewDynamicIndexRangeScanAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewMaterializedTableFunctionAccessPath(
    ...
    AccessPath *table_path) {
  //printf("access_path.h\n *NewMaterializedTableFunctionAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewUnqualifiedCountAccessPath(THD *thd) {
  //printf("access_path.h\n *NewUnqualifiedCountAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewTableValueConstructorAccessPath(THD *thd) {
  //printf("access_path.h\n *NewTableValueConstructorAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewNestedLoopSemiJoinWithDuplicateRemovalAccessPath(
  ...
  //printf("access_path.h\n *NewNestedLoopSemiJoinWithDuplicateRemovalAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewFilterAccessPath(THD *thd, AccessPath *child,
                                       Item *condition) {
  //printf("access_path.h\n *NewFilterAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewAggregateAccessPath(THD *thd, AccessPath *child,
                                          bool rollup) {
  //printf("access_path.h\n *NewAggregateAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewTemptableAggregateAccessPath(
  ...
  //printf("access_path.h\n *NewTemptableAggregateAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewLimitOffsetAccessPath(THD *thd, AccessPath *child,
                                            ...
                                            bool reject_multiple_rows,
                                            ha_rows *send_records_override) {
  //printf("access_path.h\n *NewLimitOffsetAccessPath() -> AccessPath \n"); // hong
...
inline AccessPath *NewFakeSingleRowAccessPath(THD *thd,
                                              bool count_examined_rows) {
  //printf("access_path.h\n *NewFakeSingleRowAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewZeroRowsAccessPath(THD *thd, AccessPath *child,
                                         const char *cause) {
  //printf("access_path.h\n *NewZeroRowsAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewStreamingAccessPath(THD *thd, AccessPath *child,
  ...
  //printf("access_path.h\n *NewStreamingAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
SingleMaterializeQueryBlock(THD *thd, AccessPath *path, int select_number,
                            ...
                            Temp_table_param *temp_table_param) {
  //printf("access_path.h\n SingleMaterializeQueryBlock() -> Mem_root_array<MaterializePathParameters::QueryBlock> \n"); // hong
  assert(path != nullptr);
...
inline AccessPath *NewMaterializeAccessPath(
    ...
    bool reject_multiple_rows) {
  //printf("access_path.h\n *NewMaterializeAccessPath() -> AccessPath \n"); // hong
  MaterializePathParameters *param =
...
inline AccessPath *NewMaterializeInformationSchemaTableAccessPath(
  ...
  //printf("access_path.h\n *NewMaterializeInformationSchemaTableAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewAppendAccessPath(
  ...
  //printf("access_path.h\n *NewAppendAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewWindowAccessPath(THD *thd, AccessPath *child,
  ...
  //printf("access_path.h\n *NewWindowAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewWeedoutAccessPath(THD *thd, AccessPath *child,
                                        SJ_TMP_TABLE *weedout_table) {
  //printf("access_path.h\n *NewWeedoutAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewRemoveDuplicatesAccessPath(THD *thd, AccessPath *child,
                                                 Item **group_items,
                                                 int group_items_size) {
  //printf("access_path.h\n *NewRemoveDuplicatesAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewRemoveDuplicatesOnIndexAccessPath(
    ...
    unsigned loosescan_key_len) {
  //printf("access_path.h\n *NewRemoveDuplicatesOnIndexAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewAlternativeAccessPath(THD *thd, AccessPath *child,
                                            AccessPath *table_scan_path,
                                            Index_lookup *used_ref) {
  //printf("access_path.h\n *NewAlternativeAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewInvalidatorAccessPath(THD *thd, AccessPath *child,
                                            const char *name) {
  //printf("access_path.h\n *NewInvalidatorAccessPath() -> AccessPath \n"); // hong
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline unique_ptr_destroy_only<RowIterator> CreateIteratorFromAccessPath(
  ...
  //printf("access_path.h\n CreateIteratorFromAccessPath() -> unique_ptr_destroy_only<RowIterator> \n"); // hong
