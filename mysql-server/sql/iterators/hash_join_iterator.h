...
class HashJoinIterator final : public RowIterator {
 public:
  ...
  void SetNullRowFlag(bool is_null_row) override {
    //printf("hash_join_iterator.h\nclass HashJoinIterator - SetNullRowFlag() .. for override \n"); // hong
    m_build_input->SetNullRowFlag(is_null_row);
    m_probe_input->SetNullRowFlag(is_null_row);
  }

  void EndPSIBatchModeIfStarted() override {
    //printf("hash_join_iterator.h\nclass HashJoinIterator - EndPSIBatchModeIfStarted() .. for override \n"); // hong
    m_build_input->EndPSIBatchModeIfStarted();
    m_probe_input->EndPSIBatchModeIfStarted();
  }

  void UnlockRow() override {
    //printf("hash_join_iterator.h\nclass HashJoinIterator - UnlockRow() .. for override \n"); // hong
    // Since both inputs may have been materialized to disk, we cannot unlock
    // them.
  }

  int ChunkCount() { 
    //printf("hash_join_iterator.h\nclass HashJoinIterator - ChunkCount() -> int \n"); // hong
    return m_chunk_files_on_disk.size(); }
 ...
 private:
  ...
  // hong
  bool ReadRowFromProbeIterator_Original();
  ...
  bool on_disk_hash_join() const { 
    //printf("hash_join_iterator.h\nclass HashJoinIterator - on_disk_hash_join() -> bool \n"); // hong
    return !m_chunk_files_on_disk.empty(); }
  ...
  bool RejectDuplicateKeys() const {
    //printf("hash_join_iterator.h\nclass HashJoinIterator - RejectDuplicateKeys() -> bool \n"); // hong
  ...
  // hong
  int ReadNextJoinedRowFromHashTable_Original();
