...
//unsigned long long int hash_join_chunk_constructor_count = 0; // hong
//unsigned long long int hash_join_chunk_destructor_count = 0; // hong
...
HashJoinChunk::HashJoinChunk(HashJoinChunk &&other)
  ...
  //Count_Up(&hash_join_chunk_constructor_count); // hong
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - HashJoinChunk() .. Constructor \n"); // hong
...
HashJoinChunk &HashJoinChunk::operator=(HashJoinChunk &&other) {
  ...
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - &HashJoinChunk::operator=(HashJoinChunk &&other) .. move assignment operator \n"); // hong
  m_tables = std::move(other.m_tables);
  m_num_rows = other.m_num_rows;
  m_uses_match_flags = other.m_uses_match_flags;
...
HashJoinChunk::~HashJoinChunk() { 
  //Count_Up(&hash_join_chunk_destructor_count); // hong
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - ~HashJoinChunk() .. Destructor \n"); // hong
  close_cached_file(&m_file); }

bool HashJoinChunk::Init(const TableCollection &tables, bool uses_match_flags) {
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - Init() -> bool \n"); // hong
  m_tables = tables;
...
bool HashJoinChunk::Rewind() {
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - Rewind() -> bool \n"); // hong
...
bool HashJoinChunk::WriteRowToChunk(String *buffer, bool matched) {
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - WriteRowToChunk() -> bool \n"); // hong
...
bool HashJoinChunk::LoadRowFromChunk(String *buffer, bool *matched) {
  //printf("hash_join_chunk.cc\nclass HashJoinChunk - LoadRowFromChunk() -> bool \n"); // hong
  if (m_uses_match_flags) {
