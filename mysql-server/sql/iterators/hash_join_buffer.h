...
class KeyHasher {
 public:
  ...
  size_t operator()(hash_join_buffer::Key key) const {
    //printf("hash_join_buffer.h\nclass KeyHasher - operator1() -> size_t \n"); // hong
    return robin_hood::hash_bytes(key.data(), key.size());
  }

  // hong
  size_t operator()(ImmutableStringWithLength key) const {
    //printf("hash_join_buffer.h\nclass KeyHasher - operator2() -> size_t \n"); // hong

    clock_gettime(CLOCK_MONOTONIC, &the_time_2[0]);
    size_t result = operator_orginal(key);
    clock_gettime(CLOCK_MONOTONIC, &the_time_2[1]);

    Time_Check(the_time_2, &hash_join_buffer_store_row_time, &key_hasher_operator_2_count);
        
    return result;
  }

  // hong
  size_t operator_orginal(ImmutableStringWithLength key) const {
...
class HashJoinRowBuffer {
 public:
  ...
  // hong
  StoreRowResult StoreRow_Original(THD *thd, bool reject_duplicate_keys);

  size_t size() const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - size() -> size_t \n"); // hong
    return m_hash_map->size(); }

  bool empty() const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - empty() -> bool \n"); // hong
    return m_hash_map->empty(); }

  bool inited() const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - inited() -> bool \n"); // hong
    return m_hash_map != nullptr; }
  ...
  hash_map_iterator find(const Key &key) const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - find() -> hash_map_iterator \n"); // hong
    return m_hash_map->find(key); }

  hash_map_iterator begin() const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - begin() -> hash_map_iterator \n"); // hong
    return m_hash_map->begin(); }

  hash_map_iterator end() const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - end() -> hash_map_iterator \n"); // hong
    return m_hash_map->end(); }

  LinkedImmutableString LastRowStored() const {
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - LastRowStored() -> LinkedImmutableString \n"); // hong
    assert(Initialized());
    return m_last_row_stored;
  }

  bool Initialized() const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - Initialized() -> bool \n"); // hong
    return m_hash_map.get() != nullptr; }

  bool contains(const Key &key) const { 
    //printf("hash_join_buffer.h\nclass HashJoinRowBuffer - contains() -> bool \n"); // hong
    return find(key) != end(); }
 ...
 private:
  ...
  // hong
  LinkedImmutableString StoreLinkedImmutableStringFromTableBuffers_Original(
      LinkedImmutableString next_ptr, bool *full);
};
